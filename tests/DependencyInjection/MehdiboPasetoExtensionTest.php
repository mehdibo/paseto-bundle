...
class MehdiboPasetoExtensionTest extends TestCase
{
    ...
    {
        return [
            'Non-hex string' => ["This is not a hex", "'mehdibo_paseto.secret_keys.symmetric_key' must be a hex encoded key"],
            'Invalid hex' => ["16236", "Hexadecimal input string must have an even length"],
            'Insufficient length' => [bin2hex(random_bytes(10)), "'mehdibo_paseto.secret_keys.symmetric_key' must be 32 bytes long, 10 found"]
        ];
    }
    ...
    {
        return [
            'Non-hex string' => ["This is not a hex", "'mehdibo_paseto.secret_keys.asymmetric_key' must be a hex encoded key"],
            'Invalid hex' => ["16236", "Hexadecimal input string must have an even length"],
            'Insufficient length' => [bin2hex(random_bytes(10)), "'mehdibo_paseto.secret_keys.asymmetric_key' must be 32 or 64 bytes long, 10 found"]
        ];
    }
